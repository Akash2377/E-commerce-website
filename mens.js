...
function displayData(mensData) {
  mensData.map(function (el) {
    var input1 = `<img id="photo" src="${el.image_url}" alt="" />
        <h4 id="productName">${el.name}</h4>
        <div id="price">
          <h4 id="Dprice">&#8377 ${el.price}</h4>
          <h4>&#8377 ${el.strikedoffprice}</h4>
        </div>
        <button id="addcart" onclick="addToCart(${el.id})">Add to cart</button>`;
    var div = document.createElement("div");
    div.innerHTML = input1;
    document.getElementById("womensPage").append(div);
  });
}
